...
class OBS:
    ...
    async def set_scene(self, scene: str) -> None:
        if scene != self.current_scene:
            logger.info(f"Setting current OBS scene to {scene}")
            await self.ws.call(simpleobsws.Request("SetCurrentProgramScene", {"sceneName": scene}))
    ...
    async def _watch_connection(self) -> None:
        ...
        while True:
            if not self.connected and was_connected:
                logger.debug("Connection to OBS lost")
                was_connected = False
    ...
    async def _handle_event(self, event: dict[str, Any]) -> None:
        logger.debug(f"OBS event received: {event}")
        if event["eventType"] == "ConnectionEstablished":
            self.current_scene = (await self.ws.call(simpleobsws.Request("GetCurrentProgramScene"))).responseData[
                "currentProgramSceneName"
            ]
            self.streaming = (await self.ws.call(simpleobsws.Request("GetStreamStatus"))).responseData["outputActive"]
            self.recording = (await self.ws.call(simpleobsws.Request("GetRecordStatus"))).responseData["outputActive"]
